...
- **C1 — Exact-first comparison.** The method computes the complete difference
  between two selected four-field record sets before adding hierarchy
  organization.
...
- **C3 — Fixed-condition evidence.** The evaluated pair is characterized by its
  exact difference, region distribution, second comparison, and four stated
  validation checks.
...
- 115 aligned regions: 44 added, 30 removed, one changed, 40 unchanged;
...
1. **T1 — Input and release integrity:** verify the release manifest, input
   ...
   closure.
2. **T2 — Exact baseline:** compute additions, removals, and common-path field
   labels from the complete selected-record union.
3. **T3 — Hierarchy regions:** find authored Reference/Payload landmarks,
   ...
   residual region, and align region starting paths across states.
...
every generated byte. Any numerical, label, ordering, privacy, or release-file
